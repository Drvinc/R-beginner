...
shinyServer(function(input, output, session) {
  observe({
    if(input$select_none > 0){
      if(input$select_none %% 2 == 0){
        updateCheckboxGroupInput(session,"Area",selected=c(1:12))
      }
      else{
        updateCheckboxGroupInput(session,"Area",selected=c())
      } 
    }
  })
  #TAB1
  ...
  output$TPEitems <- renderPlot({
    ...
    x <- tarr$MetPr
    tarr$diff <- with(tarr, (MetPr-min(x))/(max(x)-min(x)))
    ...
    p <- p + scale_color_gradient2(low="black",mid="blue",high = "red",midpoint = 0.4)
  ...
  output$MetPrHist <- renderPlot({
    ...
    if(A_select != 0){tarr <- tarr1}
    df = subset(tarr, select=c("Area","MetPr","YyySe"))
  ...
  output$MetPrTrend <- renderPlot({
    ...
    for (i in 1:length(YSeRange)){
      tarrM <- tarr2[(tarr$YyySe == YSeRange[i]),]
